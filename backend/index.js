...
app.get("/", (req, res) => {
  res.send("Welcome User, Server is deployed!");
});
...
app.post("/songSearch", async (req, res) => {
  ...
  try {
    ...
    const spotifySearchUrl = `https://api.spotify.com/v1/search?type=track&q=${encodeURIComponent(
      query
    )}`;
    ...
    const SpotifySearchAPI_res = await fetch(
      spotifySearchUrl,
...
app.post("/songName", async (req, res) => {
  ...
  try {
    const accessToken = await getAccessToken();
    const spotifyArtistUrl = `https://api.spotify.com/v1/artists/${encodeURIComponent(
      artistId
    )}`;
    ...
    const SpotifyArtistAPI_res = await fetch(
      spotifyArtistUrl,
      spotifyArtistOptions
    );
    const SpotifyArtistAPI_data = await SpotifyArtistAPI_res.json();
    const trueGenres = await SpotifyArtistAPI_data.genres;

    const spotifyAudioFeaturesUrl = `https://api.spotify.com/v1/audio-features/${encodeURIComponent(
      trackId
    )}`;
    ...
    const SpotifyAudioFeaturesAPI_res = await fetch(
      spotifyAudioFeaturesUrl,
      spotifyAudioFeaturesOptions
    );
    const SpotifyAudioFeaturesAPI_data =
      await SpotifyAudioFeaturesAPI_res.json();
    const AudioFeatures = {
